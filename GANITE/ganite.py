...
import numpy as np
from utils.utils import xavier_init, batch_generator


def ganite(train_x, train_t, train_y, val_x, test_x, parameters):
    """GANITE module.
  ...
  """
    ...
    for it in range(iterations):
        ...
        X_mb, T_mb, Y_mb = batch_generator(train_x, train_t, train_y, batch_size)
        # print(X_mb.shape)
        _, I_loss_curr, a = sess.run([I_solver, I_loss, Y_hat_logit], feed_dict={X: X_mb, T: T_mb, Y: Y_mb})
        # print(a.shape)
    ...
    train_y_hat = sess.run(Y_hat, feed_dict={X: train_x})
    val_y_hat = sess.run(Y_hat, feed_dict={X: val_x})
    test_y_hat = sess.run(Y_hat, feed_dict={X: test_x})
    # print(test_y_hat.shape)

    return train_y_hat, val_y_hat, test_y_hat
