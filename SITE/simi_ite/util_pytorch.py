...
def load_data(fname, FLAGS):
    ...
    if fname[-3:] == 'npz':
        ...
        data = {}
        if FLAGS.sparse > 0:
            data_in = pd.read_csv(open(fname + '.y', "rb"), delimiter=",", header=None).to_numpy()
            x = load_sparse(fname + '.x')
        else:
            data_in = pd.read_csv(open(fname, "rb"), delimiter=",", header=None).to_numpy()
            x = data_in[:, 3:]
        data['x'] = x
        data['t'] = data_in[:, 0:1]
        data['yf'] = data_in[:, 1:2]
...
def safe_sqrt(x, lbound=SQRT_CONST):
    ...
    return torch.sqrt(torch.clamp(x, lbound, float("inf")))
