...
def evaluate_cont_ate(predictions, data, i_exp, I_subset=None,
    ...
    att_pred = np.mean(eff_pred[(t > 0).reshape(-1)])
    bias_att = att_pred - np.mean(eff[(t > 0).reshape(-1)])

    atc_pred = np.mean(eff_pred[(t < 1).reshape(-1)])
    bias_atc = atc_pred - np.mean(eff[(t < 1).reshape(-1)])
...
def evaluate(output_dir, data_path_train, data_path_test=None, binary=False):
    ...
    # print(results[0]['train']['pred'].shape)
    if len(results) == 0:
    ...
    # print(eval_results)
    for k in keys:
        # print([eval_results[i]['train'][k] for i in range(len(eval_results))])
