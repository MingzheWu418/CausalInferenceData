...
def evaluate(config_file, overwrite=False, filters=None):
    ...
    if not os.path.isdir(output_dir):
        raise Exception('Could not find datasets at path: %s' % output_dir)

    if cfg['dataform'][-3:] == 'npz':
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        evaluate("configs/ihdp_pytorch.txt", 1, filters=None)
        # evaluate("configs/twins_pytorch.txt", 1, filters=None)
