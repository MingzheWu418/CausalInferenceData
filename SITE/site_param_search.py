...
def run(cfg_file, num_runs):
    ...
    for i in range(num_runs):
        ...
        # calling site_net_train.py would run the original implementation
        call('python site_net_train_pytorch.py %s' % flags, shell=True)
        # call('python site_net_train.py %s' % flags, shell=True)


if __name__ == "__main__":
    if len(sys.argv) < 3:
        # run("configs/ihdp.txt", 10)
        run("configs/ihdp_pytorch.txt", 10)
