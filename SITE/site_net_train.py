...
from simi_ite.evaluation import pehe_nn
...
tf.app.flags.DEFINE_integer('pred_output_delay', -1,
                            """Number of iterations between prediction outputs. (-1 gives no intermediate datasets). """)
...
tf.app.flags.DEFINE_boolean('split_output', 0, """Whether to split datasets layers= between treated and control. """)
...
tf.app.flags.DEFINE_string('propensity_dir', './propensity_score/ihdp_propensity_model.sav',
                           """Dir where the propensity model is saved""")
...
def train(SITE, sess, train_step, D, I_valid, D_test, logfile, i_exp):
    ...
    if D['HAVE_TRUTH']:
        ...
        # print sess.run(SITE.sample_weight)
        # print cf_error
    ...
    for i in range(FLAGS.iterations):
        ...
        if i % FLAGS.output_delay == 0 or i == FLAGS.iterations - 1:
            # obj_loss, f_error, pddm_loss_batch, mid_dist_batch, risk = sess.run(
            #     [SITE.tot_loss, SITE.pred_loss, SITE.pddm_loss, SITE.mid_distance, SITE.risk],
            #     feed_dict=dict_factual)
            obj_loss, f_error, pddm_loss_batch, mid_dist_batch = sess.run(
                [SITE.tot_loss, SITE.pred_loss, SITE.pddm_loss, SITE.mid_distance],
                feed_dict=dict_factual)
        ...
        if (FLAGS.pred_output_delay > 0 and i % FLAGS.pred_output_delay == 0) or i == FLAGS.iterations - 1:
            ...
            # print(pehe_nn(y_pred_f, y_pred_cf, D['yf'], D['x'], D['t']))
